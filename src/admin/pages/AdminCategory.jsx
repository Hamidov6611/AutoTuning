...
const AdminCategory = () => {
  ...
  const getData = async () => {
    try {
      const { data } = await instance(`/category?page=${pageId}&limit=10`);
    ...
    <AdminLayout>
      <div className="min-h-[100vh] py-16 md:pl-[25px]">
        <div className=" w-[90%] lg:w-[98%] mx-auto mb-12 ml-[6%] sm:ml-[1%] items-center mt-2 flex justify-between">
          <p className="text-white font-semibold text-[20px]">Категория ({pageSize} шт.)</p>
          <IconButton
        ...
        <div className="rounded-sm  border-stroke bg-[#24303f] w-[98%] mx-auto px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
          <div className="max-w-full overflow-x-auto">
            <table className="w-full table-auto">
              ...
              {isEdit && (
                <EditCategory setIsOpen={setIsEdit} getData={getData} id={itemId} />
              )}
