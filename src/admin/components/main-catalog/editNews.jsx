...
const EditMainCatalog = ({ setIsOpen, getData, id }) => {
  const [name, setName] = useState("");
  const [file, setFile] = useState(null);
  const [text, setText] = useState("");
  const closeHandler = () => setIsOpen(false);
  const EditMainCatalog = async (e) => {
    e.preventDefault();
    try {
      const data = new FormData();
      data.append("img", file);
      data.append("title", name);
      await instance.patch(`/catalog/${id}/`, data);
      getData();
      setIsOpen(false);
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      <form
        onSubmit={EditMainCatalog}
        className="rounded-md w-[90%] md:w-[50%] p-4 overflow-y-auto lg:w-[30%] max-h-[340px] bg-white"
      >
