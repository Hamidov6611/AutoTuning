...
const AddMainCatalog = ({ setIsOpen, getData }) => {
  const [name, setName] = useState("");
  const [file, setFile] = useState(null);
  const closeHandler = () => setIsOpen(false);
  const AddMainCatalog = async (e) => {
    e.preventDefault();
    try {
      const data = new FormData();
      data.append("img", file);
      data.append("title", name);
      await instance.post(`/catalog/add`, data);
      getData();
      setIsOpen(false);
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal2 flex items-center justify-center z-50">
      <form
        onSubmit={AddMainCatalog}
        className="rounded-md w-[90%] md:w-[50%] p-4 lg:w-[30%] overflow-y-auto max-h-[340px] bg-white"
      >
