...
const AddBrand = ({ setIsOpen, getData, id }) => {
  ...
  const AddBrand = async (e) => {
    e.preventDefault();
    try {
      const data = { year_id: yearId, title: name };
      await instance.post(`/brand-engine`, data);
      getData();
      setIsOpen(false);
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      <form
        onSubmit={AddBrand}
        className="rounded-md w-[90%] md:w-[50%] p-4 overflow-y-auto lg:w-[30%] max-h-[460px] bg-white"
      >
        <div className="w-full flex items-center justify-between text-[#343434] font-semibold text-[16px]">
          <p>Добавить Брендовый двигатель</p>
          <svg
