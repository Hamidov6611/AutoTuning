...
const RemoveBrand = ({ setIsRemove, getData, id }) => {
  const closeHandler = () => setIsRemove(false);
  const RemoveBrand = async (e) => {
    e.preventDefault();
    try {
      await instance.delete(`/brand-engine/${id}/`);
      getData();
      setIsRemove(false);
    ...
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      ...
      >
        <div className="w-full flex items-center justify-between text-[#343434] font-semibold text-[16px]">
          <p>Удалить Брендовый двигатель</p>
          <svg
        ...
        <button
          type="submit"
          className="w-full my-8 bg-[#343434] text-white font-semibold py-2 rounded-lg"
        >
          Удалить
        </button>
      </form>
