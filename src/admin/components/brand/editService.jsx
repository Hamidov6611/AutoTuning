...
const EditBrand = ({ setIsOpen, getData, id }) => {
  ...
  const EditBrand = async (e) => {
    ...
    try {
      ...
      data.append("catalog_id", categoryId);
      await instance.patch(`/brand/${id}/`, data);
      getData();
      setIsOpen(false);
  ...
  const getPrevData = async () => {
    try {
      ...
      setName(data?.title);
      setFile(data?.img);
      setCategory(data?.catalog.title);
      setCategoryId(data?.catalog?.id);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const getAll = async () => {
    const { data } = await instance.get(`/catalog/?page=1&limit=20`);
    setAllCat(data);
    console.log(data);
  };
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      <form
        onSubmit={EditBrand}
        className="rounded-md w-[90%] md:w-[50%] p-4 overflow-y-auto lg:w-[30%] max-h-[360px] bg-white"
      >
        ...
        <div className="flex w-full my-4">
          ...
          >
            <option className="text-transparent hover:bg-transparent">
              {category}
            </option>
