...
const AddBrand = ({ setIsOpen, getData, id }) => {
  ...
  const AddBrand = async (e) => {
    e.preventDefault();
    try {
      await instance.post(`/stage2`, formData);
      getData();
      setIsOpen(false);
    ...
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-x-4 mb-4">
          ...
          <div className="flex w-full">
            <input
              value={formData.original_nm}
              onChange={(e) =>
                setFormData({ ...formData, original_nm: e.target.value })
              }
              type="text"
              className="outline-none border w-full py-2 border-[#343434] rounded-md text-[#343434] px-3 tetx-[15px]"
              placeholder="Original (NM)"
              required
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 mb-4">
          <div className="flex w-full">
            <input
              value={formData.original_hp}
              onChange={(e) =>
                setFormData({ ...formData, original_hp: e.target.value })
              }
              type="text"
              className="outline-none border w-full py-2 border-[#343434] rounded-md text-[#343434] px-3 tetx-[15px]"
              placeholder="Original (HP)"
              required
            />
          </div>
          <div className="flex w-full">
            <input
              value={formData.after_hp}
              onChange={(e) =>
                setFormData({ ...formData, after_hp: e.target.value })
              }
              type="number"
              className="outline-none border w-full py-2 border-[#343434] rounded-md text-[#343434] px-3 tetx-[15px]"
              placeholder="After (HP)"
              required
            />
          </div>
        </div>
