...
const EditNews = ({ setIsOpen, getData, id }) => {
  ...
  const EditNews = async (e) => {
    ...
    try {
      ...
      await instance.patch(`/auto-model/${id}/`, data);
      getData();
      setIsOpen(false);
  ...
  const getPrevData = async () => {
    try {
      const { data } = await instance.get(`/auto-model/${id}/`);
      console.log(data);
      setName(data?.title);
      setFile(data?.img);
    } catch (error) {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      <form
        onSubmit={EditNews}
        className="rounded-md w-[90%] md:w-[50%] p-4 overflow-y-auto lg:w-[30%] min-h-[140px] bg-white"
      >
