...
const RemoveStock = ({ setIsRemove, getData, id }) => {
  const closeHandler = () => setIsRemove(false);
  const RemoveStock = async (e) => {
    e.preventDefault()
    try {
        await instance.delete(`/stock/${id}/`)
        getData()
        setIsRemove(false)
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      <form onSubmit={RemoveStock} className="rounded-md w-[90%] md:w-[50%] p-4 lg:w-[30%] h-[140px] bg-white">
