...
const AddBrand = ({ setIsOpen, getData, id }) => {
  ...
  const [dates, setData] = useState({catalog: [], brand: [], model: []})
  const closeHandler = () => setIsOpen(false);
  const AddBrand = async (e) => {
    e.preventDefault();
    try {
      const data = {model_id: modelId, title: name}
      await instance.post(`/brand-year`, data);
      getData();
      setIsOpen(false);
